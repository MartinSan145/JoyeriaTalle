function Nuevo() {
    ...
    $("#btnsalir").attr("disabled", "true");
        $("#operacion").val("1");
      $.post("autonumerico", {campo: "idmarca_tarjeta", tabla: "marca_tarjeta"})
            .done(function (data) {
                $("#txtcodigo").val(data);
            });
...
function borrar() {
    ...
    var sql = "";
        ...
        var conf = "";
     if ($("#operacion").val() === "3") {
            sql = "delete from marca_tarjeta where idmarca_tarjeta = " + $("#txtcodigo").val();
...
function grabar() {
    ...
    if ( a === "") {
        ...
        var conf = "";
         if ($("#operacion").val() === "1") {
            sql = "insert into marca_tarjeta values(" + $("#txtcodigo").val() + ",upper('" + $("#txtdescripcion").val() + "'))";
            conf = "¿DESEA GRABAR LA NUEVA MARCA?";
            men = "LA NUEVA MARCA FUE REGISTRADA CON EXITO";
        }
        if ($("#operacion").val() === "2") {
            sql = "update marca_tarjeta set descri_marca = '" + $("#txtdescripcion").val() + "' where idmarca_tarjeta = " + $("#txtcodigo").val();
...
function cargar_tabla(filtro) {
    var sql = "select * from marca_tarjeta where descri_marca ilike '%"+filtro+"%' order by idmarca_tarjeta";
    $.post("get_marca", {sql: sql})
            .done(function (data) {
